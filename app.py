...
def predict():
    ...
    if file:
        ...
        predictions = {name: model.predict(features_scaled) for name, model in models.items()}
        decoded_predictions = {name: label_encoder.inverse_transform(pred)[0] for name, pred in predictions.items()}
